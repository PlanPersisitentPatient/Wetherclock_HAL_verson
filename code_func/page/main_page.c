...
/**
 * @brief  主页面初始化绘制（开机/切换页面时调用，绘制全屏布局、背景、默认值）
 * @param  无
 * @return 无
 */
void main_page_display(void)
{
    ...
    do {
        ...
        // 显示默认占位日期
        st7789_write_string(35, 121, "----/--/-- 星期四", mkcolor(143, 143, 143), color_bg_time, &font20_maple_bold);
    } while (0);
    ...
    do {
        ...
        // 显示城市名称：合肥
        main_page_redraw_outdoor_city("广州");
        // 显示默认室外温度（无效值）
...
/**
 * @brief  刷新时间显示（时分，冒号秒闪）
 * @param  time: RTC时间结构体指针，包含时、分、秒
 * @return 无
 */
void main_page_redraw_time(rtc_date_time_t *time)
{
    char str[6];
    // 秒数偶数显示冒号，奇数显示空格，实现闪烁效果
...
/**
 * @brief  刷新日期+星期显示
 * @param  date: RTC日期结构体指针，包含年、月、日、星期
 * @return 无
 */
...
void main_page_redraw_inner_temperature(float temperature)
{
    ...
    // 显示室内温度
    st7789_write_string(30, 192, str, mkcolor(0, 0, 0), color_bg_inner, &font32_maple_bold);
}

/**
 * @brief  刷新室内湿度显示
 * @param  humidity: 室内湿度值（浮点型）
 * @return 无
 */
void main_page_redraw_inner_humidity(float humidity)
{
    ...
    // 显示室内湿度
    st7789_write_string(25, 239, str, mkcolor(0, 0, 0), color_bg_inner, &font32_maple_bold);
}
...
void main_page_redraw_outdoor_temperature(float temperature)
{
    ...
    if (temperature > -10.0f && temperature <= 100.0f)
        snprintf(str, sizeof(str), "%2.0f", temperature);
    // 显示室外温度
    st7789_write_string(135, 190, str, mkcolor(0, 0, 0), color_bg_outdoor, &font32_maple_bold);
}

/**
 * @brief  根据天气代码刷新天气图标
 * @param  code: 天气状态代码（数字，对应晴/阴/雨/雪等）
 * @return 无
 */
...
//    char comma = (time->second % 2 == 0) ? ':' : ' ';
//    // 格式化时间为 HH:MM 格式
//    snprintf(str, sizeof(str), "%02u%c%02u", time->minute, comma, time->second);
//    // 显示时间
//    st7789_write_string(25, 42, str, mkcolor(0, 0, 0), color_bg_time, &font76_maple_extrabold);
//}